...
#-----------------------------------------------------------------------------
vendor_saved_apps := FMRadio LenovoFMRadio MtkBt  	advancesettings LenovoLightSettings SchedulePowerOnOff MIPop
...
#-----------------------------------------------------------------------------
baidu_modify_apps := Phone SystemUI Settings
...
ziptophone: 
	...
	adb push out/ota*.zip /sdcard/update.zip
...
flash-ota: 
	adb shell rm /sdcard/update.zip
	adb push out/ota*.zip /sdcard/update.zip
	#adb shell su -c "rm /data/dalvik-cache/*"
	adb shell su -c "echo --wipe_data >> /cache/recovery/command"
